...
Selected-card marker/category/visibility shortcuts enter through a separate
typed controller action rather than pretending a context menu is open. The DOM
keymap emits only digit/Alt-digit/`0`, `Y`/Alt-`Y`, `W`, Alt-`E`/`T`/`P`, `C`,
and `Z`/Alt-`Z` requests with the current stable card ID. The controller
requires that ID to be the installed selection on a ready live-player
projection, then reuses the bounded stack, annotation, private-inspection, or
public-visibility resolver. Accepted ability/category/removal gestures clear
selection in the same reduction; additive damage, condition cycling, and all
visibility gestures retain it. Missing-marker Alt-digit and Alt-`Y` keep the
...
five move choices, and all twelve selected-card shortcut commands; native prompt
