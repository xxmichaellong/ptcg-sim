...
| Card movement         | `MoveCard`, `MoveCards`, `MoveZoneContents`, `ResolveLooseBoardCards`, `MoveCardToDeckTop`, `MoveCardToDeckBottom`, `ShuffleCardIntoDeck`, `SwapCardWithDeckTop`, `MoveCardToStadium`, `MovePrizesToDeckBottom` | Broad loose-board actions require an exact ordered source snapshot; singleton stadium replacement pins the current occupant and executes in one normalized transaction. |
...
idempotent target-room initialization and cross-object orchestration,
...
and admission authority; the save adapter durably encrypts one immutable plan
and its completion receipt under the original expiry. See
