...
| `pnpm run build:v2`                           | Build Worker and web artifacts, then verify provenance, credential exclusion, fixture exclusion, and emitted card-back bytes. |
...
## Explicit continuation preview

`playwright.continuation.config.ts` owns an isolated built Worker origin on port
4176 and executes only `tests/browser/continuation-full-stack.spec.ts`. Its
deterministic test activation token, AES key, and four-by-four quota policy are
passed solely as `wrangler dev --var` arguments, so the deployable
`wrangler.jsonc` remains default-off and contains no key or capacity default.
The three-context Chromium journey proves a real two-player checkpoint can be
downloaded as a strict capability-only `.ptcgsave`, uploaded through the native
file chooser, restored into a distinct ready room at the exact match revision,
and handed to the opponent through the ordinary foreground clipboard
invitation. It then proves the target advances independently, the source
opponent remains on the unchanged source fork, the completed save is revoked,
and no bearer enters request URLs, DOM, location, local storage, or session
storage.

`PTCGSIM_CONTINUATION_PREVIEW_URL=https://... pnpm run
test:continuation:browser` suppresses local server startup and runs the same
journey against an explicitly provisioned managed preview. The ordinary CI lane
...
audit fails on any reported severity and also fails closed when the advisory
service cannot be reached. The build runs with a fake keyring sentinel; the
artifact gate rejects that exact value, sensitive environment values available
to the build that are at least 16 bytes, and recognizable private-key/access-token
formats without printing the matched credential. The updated legacy dependency
graph is additionally exercised through a child-process smoke test covering its
rendered root, SQLite-backed import lookup, and a real Socket.IO player admission.
Only after the quality job succeeds does the `chromium` job install
...
- Schema-v7 admission now makes solo replay disclosure structurally safe by
  ...
  both-side reset, alternate deck/back authority, live opponent-hand disclosure,
  local revision-neutral cover/uncover, an accepted opponent-hand move, and
  same-runtime tab parking. Authority tests separately prove spectators and
  multiplayer stay concealed and canonical IDs never enter the projection; the
  default route and v1 remain unchanged.
