...
| ADR-018 | `PROVISIONAL`      | Keep room code UX but authorize with high-entropy seat/resume capabilities and one-time WebSocket tickets.                                   | Implemented with a strict same-origin POST, digest-only bounded 30-second ticket registry, atomic issue/redemption journals, fresh resume rotation, ticket-only initial Hello, safe browser bootstrap, v4→v5 migration, and replay/expiry/mismatch/failure tests. Visible create/join wiring and production abuse/load evidence remain.                                                                                                                          |
