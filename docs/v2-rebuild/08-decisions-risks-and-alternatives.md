...
| ADR-005 | `SPIKE_REQUIRED`   | Prefer Worker + one Durable Object per room; compare Colyseus if deployment constraints justify it.                                         | Room locality and MagicCircle expertise are strong, but durability, hibernation, tooling, cost, and portability require evidence. |
...
| ADR-011 | `PROPOSED`         | Separate authoritative view, pending overlay, and presentation stores; use a small external store (Zustand vanilla acceptable).             | Prevents React/Pixi from becoming a second truth.                                                                                 |
...
| ADR-014 | `PRODUCT_REQUIRED` | Preserve solo-only undo for first v2; decide checkpoints, hidden randomness, and future multiplayer consent separately.                     | Current UI documents undo as one-player only; rebuild must not add UX scope.                                                      |
...
| ADR-016 | `PROPOSED`         | Persist resolved domain events and periodic canonical checkpoints; commands remain dedupe/audit metadata, timeline is projected separately. | Replay survives future decision/randomness changes and safe logs do not expose hidden facts.                                      |
