...
| ADR-018 | `PROVISIONAL`      | Keep room code UX but authorize with high-entropy seat/resume capabilities and one-time WebSocket tickets.                                       | Implemented with strict creation/invitation/ticket POSTs, distinct credential validation, bounded digest-only invitation and 30-second ticket registries, retry rotation, atomic issue/redemption journals, fresh resume rotation, ticket-only initial Hello, safe creator/guest bootstraps, v4/v5→v6 migrations, layered creation/room request budgets, five-minute unclaimed cleanup, and failure tests. Cross-browser presentation is split into ADR-020; preview abuse/load evidence remains. |
