...
| ADR-018 | `PROVISIONAL`      | Keep room code UX but authorize with high-entropy seat/resume capabilities and one-time WebSocket tickets.                                    | Implemented with strict creation/ticket POSTs, distinct credential validation, digest-only bounded 30-second ticket registry, atomic issue/redemption journals, fresh resume rotation, ticket-only initial Hello, safe creator/join bootstraps, v4→v5 migration, and failure tests. Cross-browser handoff is split into ADR-020; production abuse/load evidence remains.                                                                                         |
...
| ADR-020 | `PRODUCT_REQUIRED` | Select how anonymous player-two and spectator invitation capabilities move between browsers while preserving the familiar room-code workflow. | The creator path now holds credentials in one-time non-serializing memory, but v1's shared room ID cannot remain authorization under SEC-001/SEC-003. Choose an explicit high-entropy invitation transfer, trusted account-mediated claim, or another audited channel before lobby wiring; raw bearer values in URLs, storage, analytics, React state, or hidden DOM are rejected.                                                                               |
...
9. For anonymous multiplayer, how should the second player and spectators
   receive their high-entropy invitation while the lobby still looks and feels
   like the current room-ID flow? This must define manual typing, copy/paste,
   repeat spectators, expiration/revocation, and whether an account-free trusted
   relay is acceptable.
...
| R-018 | Guest invitation leaks authority or failed creator bootstrap accumulates orphan rooms   | Medium / High       | ADR-020 threat model, one-time bounded claims, redacted boundaries, creation/admission rate limits, and an unclaimed-room TTL/alarm before lobby rollout. |
