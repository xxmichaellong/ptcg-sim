...
| ADR-016 | `PROVISIONAL`      | Persist resolved domain events and canonical checkpoints; project a connected session's own replay perspective and stream frames separately. | A 128-batch/512-KiB ledger, v1/v2-to-v3 migration, fresh opaque aliases, atomic client assembly, isolated deterministic playback, and request-correlated application coordination are implemented; visible UI and archival retention/export remain Phase 7. |
