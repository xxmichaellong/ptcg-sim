...
| ADR-015 | `PRODUCT_REQUIRED` | Ratify reference performance hardware and accessibility support windows.                                                                                                                              | ADR-023 resolves the first-release browser/device/viewport matrix; quantitative hardware and accessibility gates still require named environments.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
## Remaining blocking product questions

These cannot be answered purely by engineering:

ADR-021 resolves historical v1 saved-game compatibility for the first release:
no v1 action-history file or old share-link import is exposed or promised.
ADR-022 resolves the v1 fallback/deprecation observation window: eligibility
requires 30 uninterrupted days at full v2 cutover and two stable production
release cycles, followed by explicit product approval.

1. Which opponent-private manipulations are intentional tabletop features and
   which are accidental privacy leaks?
2. Which known behavioral bugs may be corrected during parity work, and who signs
   each exception?
