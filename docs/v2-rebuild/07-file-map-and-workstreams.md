...
| sidebox import/export/replay handlers                 | `room-authority/{replay-history,projected-replay}.ts`, `client-session/{session,replay-playback}.ts`, `apps/web/src/{session,replay,presentation}/*`, `features/saves/*`, `features/replay/*` | Bounded delivery, playback, coordination, board guarding, headless chrome/control binding, and mode-gated activity/accessibility/animation effects are implemented; recipient-safe per-card wording, mounting the full sidebar, and save/export/continuation remain later work. |
