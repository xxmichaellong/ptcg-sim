...
| sidebox import/export/replay handlers                 | `room-authority/{replay-history,projected-replay}.ts`, `client-session/{session,replay-playback}.ts`, `apps/web/src/{session,replay}/*`, `features/saves/*`, `features/replay/*` | Bounded delivery, playback, coordination, board guarding, and legacy-shaped replay controls are implemented; mounting the controls, save/export/continuation, and full replay chrome remain later work. |
