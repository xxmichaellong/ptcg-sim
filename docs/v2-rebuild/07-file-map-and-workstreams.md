...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, lifecycle plus `draw`, `discardAndDraw`, `shuffleAndDraw`, direct prize `shuffleZone`, `moveToDeckTop`, `shuffleIntoDeck`, `switchWithDeckTop`, and `shufflePrizesToDeckBottom` tuple decoding, deterministic deck/identity/outcome adapters, and closed lifecycle/draw/discard-and-draw/shuffle-hand-and-draw/prize-shuffle/zone-backed deck-action/prizes-to-deck-bottom transaction/replay proof are implemented without route wiring. Continue frozen movement/remaining schemas before widening conversion; never import the live v1 runtime. |
