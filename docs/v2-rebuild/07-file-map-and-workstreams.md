...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, lifecycle plus `draw`, `discardAndDraw`, `shuffleAndDraw`, `shuffleBottomAndDraw`, bottom-mode and target-free loose-zone/stadium/new-play-stack/bare-whole-stack plus source-zone-targeted active/bench `moveCardBundle`, direct prize `shuffleZone`, `moveToDeckTop`, `shuffleIntoDeck`, `switchWithDeckTop`, and `shufflePrizesToDeckBottom` tuple decoding, deterministic deck/identity/outcome adapters, and closed lifecycle/draw/discard-and-draw/both hand-shuffle-and-draw forms/loose, stadium, play-stack creation/layout/attach/evolve/prize-shuffle/zone-backed deck-action/prizes-to-deck-bottom transaction/replay proof are implemented without route wiring. Numeric targets are resolved only when the flattened coordinate identifies a current stack top; rich-stack origins, targeted stack swaps, and work areas remain closed. Reachable loose-board take-turn cleanup plus owner-scoped loose/stadium/play reset are pinned. Continue frozen rich-stack/stack-departure movement and remaining schemas before widening conversion; never import the live v1 runtime. |
