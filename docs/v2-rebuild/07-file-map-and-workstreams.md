...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, lifecycle plus `draw`, `discardAndDraw`, `shuffleAndDraw`, `shuffleBottomAndDraw`, bottom-mode and target-free loose-zone/stadium/new-play-stack/rich-whole-stack plus source-zone-targeted, lower-card-reattachment, all stack-card departures, and stack-swapping active/bench `moveCardBundle`, direct prize `shuffleZone`, `moveToDeckTop`, `shuffleIntoDeck`, `switchWithDeckTop`, and `shufflePrizesToDeckBottom` tuple decoding, deterministic deck/identity/outcome adapters, and closed lifecycle/draw/discard-and-draw/both hand-shuffle-and-draw forms/loose, stadium, play-stack creation/layout/attach/evolve/departure/prize-shuffle/zone-backed deck-action/prizes-to-deck-bottom transaction/replay proof are implemented without route wiring. Play coordinates use newest-to-oldest evolution order followed by versioned attachment order: top sources support whole-stack movement and cross-slot swaps, numeric lower sources atomically reattach, and target-free stack cards move to loose zones with canonical top-dependent staging or independent lower/attachment removal. Work-area origins remain closed. Reachable loose-board take-turn cleanup plus owner-scoped loose/stadium/play reset are pinned. Continue frozen work-area movement and remaining schemas before widening conversion; never import the live v1 runtime. |
