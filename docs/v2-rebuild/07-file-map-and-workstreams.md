...
and teardown races. The live Solo Options menu now wires the source-shaped
`Enter replay mode` control and hidden `.json` picker to that transaction with
bounded reads, cancellation, and same-file retry. No canonical-state serializer
or resumable continuation is implied. The later production split remains:
...
apps/web/src/session/
  ...
  RemoteContinuationTransport.ts
  RemoteContinuationCustody.ts
...
| native deck-builder listener/render/sync files        | `apps/web/src/features/deck/{LegacyDeckBuilderSession,LegacyDeckBuilderWorkspace,LegacyDeckImportPanel,popular-decklists,card-back-custody}.*`, `apps/web/src/dev/LegacyDeckBuilderBrowserHarness.tsx`, `tests/browser/legacy-deck-builder-browser-parity.spec.ts` | React DOM with independent main/alternate state and the unchanged visible workflow. The exact 168-deck corpus from `sample.decklists.js` is provenance-pinned and dynamically imported only after book/wand use; the right panel holds imports in an editable Save/Cancel/Confirm review transaction. One route-neutral session owner composes both surfaces, persistent offline deck and card-back custody, dynamic solo/multiplayer capability, new-authority reinstallation, same-authority dirty-only remount flushing, acknowledged card-back-before-deck drains, one combined unload guard, the catalog, and the foreground browser image-load gate. Exact bounded arbitrary card-back strings remain direct native image URLs without an application parser, allowlist, proxy, fetch, or CORS requirement. A direct-import-only Chromium harness now compares complete v1 and candidate controls, geometry, and paint and exercises arbitrary-image, acknowledged recovery, multiplayer denial, focus, inertness, and teardown. It exposed and closed the viewport-relative panel-padding, inline-control-gap, Linux textarea-font, and panel/tab overlap-stacking differences. The opt-in `?room-lobby=1` entry activates this surface through a first-use lazy boundary while the default route remains unchanged. Exact store custody survives lobby/live/Leave and parked Solo navigation, multiplayer denies alternate edits, Solo enables them, card backs drain before decks, and production/browser gates pin lazy loading, exact arbitrary-URL publication, public-back propagation, and private-face request isolation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| sidebox import/export/replay handlers                 | `room-authority/{replay-history,projected-replay}.ts`, `client-session/{session,replay-playback}.ts`, `apps/web/src/{session,replay,presentation}/*`, `features/saves/*`, `features/replay/*`                                                                      | Bounded delivery/playback, board guarding, replay chrome, mode-gated effects, seek-synchronized activity, authenticated bounded chat transport/presentation, serial consumers, legacy activity/live-region mounting, ticket-safe bootstrap, remote-room composition, connected multiplayer Attack/Pass/flower/chat/Setup/Reset/Leave controls, p1-shaped Solo controls with Undo and acknowledged sequential Setup/Reset Both, the confirmed multiplayer-to-Solo teardown, live Solo parking/return, local live/replay Options for battle-log export, browser full screen, live-only log clearing, perspective-file export/import, and complete page-local Settings ownership including the accepted direct custom-background path are implemented; visible create/join/Solo UI remains isolated behind `?room-lobby=1`; Deck navigation is active behind `?room-lobby=1` with first-use chunk isolation and retained deck/card-back custody; canonical state import remains deferred, while server-held continuation now has a strict browser transport, private stable-retry custody, bounded capability-file handoff, player-only Save/Resume controls, and an atomic ready-target installer; its production route remains inactive.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
In the inventory row above, deferred legacy canonical state import excludes the
completed perspective-file export and wired inert
`client-session/projected-replay-file.ts` import transaction described in this
section. The live Solo route owns native selection and installation. The live
multiplayer player Options menu now wires a bounded
`.ptcgsave` capability handoff and private exact-retry restoration. The lobby
waits for a ready requester session and foreground opponent-invitation delivery
before atomically replacing the source owner; managed browser evidence remains
later release work.
