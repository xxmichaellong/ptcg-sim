...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, lifecycle plus first `draw` tuple decoding, deterministic deck/identity/outcome adapters, and lifecycle-only transactional candidate/replay proof are implemented without route wiring. Continue frozen movement/remaining schemas before widening conversion; never import the live v1 runtime. |
