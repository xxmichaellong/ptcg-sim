...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, lifecycle plus `draw`, `discardAndDraw`, `shuffleAndDraw`, `shuffleBottomAndDraw`, bottom-mode and target-free loose-zone/stadium `moveCardBundle`, direct prize `shuffleZone`, `moveToDeckTop`, `shuffleIntoDeck`, `switchWithDeckTop`, and `shufflePrizesToDeckBottom` tuple decoding, deterministic deck/identity/outcome adapters, and closed lifecycle/draw/discard-and-draw/both hand-shuffle-and-draw forms/loose and stadium movement/prize-shuffle/zone-backed deck-action/prizes-to-deck-bottom transaction/replay proof are implemented without route wiring. Reachable loose-board take-turn cleanup plus owner-scoped loose/stadium reset are pinned. Continue frozen stack/target movement and remaining schemas before widening conversion; never import the live v1 runtime. |
