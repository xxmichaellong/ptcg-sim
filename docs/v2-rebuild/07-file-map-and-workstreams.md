...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, lifecycle tuple decoder, deterministic deck/identity/outcome adapters, and lifecycle-only transactional candidate/replay proof are implemented without route wiring. Add frozen movement and remaining positional schemas before widening conversion; never import the live v1 runtime. |
