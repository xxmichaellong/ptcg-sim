...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, first lifecycle tuple decoder, deterministic order-preserving deck-definition adapter, and import-wide identity/resolved-outcome context are implemented without route wiring. Add remaining frozen positional schemas and transactional interpreters incrementally; never import the live v1 runtime. |
