...
| `setup/settings/settings.js` and settings listeners   | `apps/web/src/session/RemoteRoomSettings.tsx`, then `src/features/settings/*`                   | The isolated lobby now owns page-local preferences across lobby/live/replay and mounts source-shaped Dark mode and Hide containers controls without renderer remount, storage, or authority traffic. Its default-visible zone/stadium outline policy preserves paint-only behavior and all hit regions. Opponent-hand, custom-background, and remaining static-content parity stay separate work and must not write game state through DOM. |
