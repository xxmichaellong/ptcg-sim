...
| sidebox import/export/replay handlers                 | `room-authority/{replay-history,projected-replay}.ts`, `client-session/{session,replay-playback}.ts`, `apps/web/src/{session,replay,presentation}/*`, `features/saves/*`, `features/replay/*` | Bounded delivery/playback, board guarding, replay chrome, mode-gated effects, seek-synchronized activity, serial consumers, legacy activity/live-region mounting, and an explicit-input remote-room composition are implemented; one-time browser admission, normal sidebar/chat/navigation, and save/export/continuation remain later work. |
