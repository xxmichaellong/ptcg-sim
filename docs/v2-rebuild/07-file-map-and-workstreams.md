...
| sidebox import/export/replay handlers                 | `room-authority/{replay-history,projected-replay}.ts`, `client-session/{session,replay-playback}.ts`, `apps/web/src/{session,replay,presentation}/*`, `features/saves/*`, `features/replay/*` | Bounded delivery/playback, board guarding, headless chrome, mode-gated effects, seek-synchronized activity, transient queues, keyed feed projection, serial consumers, a concrete legacy activity/live-region surface owner, and recipient-safe reveal/hide/look wording are implemented; room-route/full-sidebar integration and save/export/continuation remain later work. |
