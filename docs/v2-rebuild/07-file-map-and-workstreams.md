...
| legacy action arrays/version                          | `packages/legacy-import/src/*`, then versioned `src/v1/*` interpreters                                                                                                                        | The strict 1.5/1.5.1 envelope parser, exact 50-action allowlist, lifecycle plus `draw` and direct prize `shuffleZone` tuple decoding, deterministic deck/identity/outcome adapters, and closed lifecycle/draw/direct-prize-shuffle transaction/replay proof are implemented without route wiring. Continue frozen movement/remaining schemas before widening conversion; never import the live v1 runtime. |
