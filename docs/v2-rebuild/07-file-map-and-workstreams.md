...
| `setup/settings/settings.js` and settings listeners   | `src/features/settings/*`                                                                       | An optional client-owned preference value now crosses the live/replay board wrapper and updates either renderer without remounting. Its default-visible zone/stadium outline policy preserves the legacy “Hide containers” paint-only behavior and all hit regions; controls, storage, and remaining appearance parity stay separate work and must not write game state through DOM. |
