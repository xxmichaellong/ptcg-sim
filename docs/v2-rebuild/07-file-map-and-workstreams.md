...
In the inventory row above, deferred canonical state import and continuation UI
exclude the completed perspective-file export and wired inert
`client-session/projected-replay-file.ts` import transaction described in this
section. The live Solo route owns native selection and installation. Canonical
continuation's browser transport/private retry owner are implemented without a
product caller; handoff and restored-room installation remain later work.
