...
Focused DOM-runtime tests cover that lifecycle. A development-only Chromium
harness now mounts the opt-in runtime on a translated 75%-scale surface and
drives native mouse-generated pointer events through normal/flipped lower
...
scaled gestures update the resized DOM surface correctly.
`BoardSessionRuntime` now exposes its retained immutable characterization
snapshot plus an isolated layout subscription for route chrome; unchanged
layout is silent, subscriber failures cannot block later subscribers, and
disposal clears every listener. `LegacyBoardChrome` consumes only that snapshot,
the local player ID, and five route callbacks. It copies the existing handles,
buttons, hover paint, and tooltip text without entering either renderer. A
development-only Chromium harness composes the real DOM candidate underneath
that chrome, uses native handles plus the visible flip/fullscreen controls, and
...
imports or enables the harness or chrome. Full control-to-game workflows and
...
The route-owned React overlay now supplies damage and special-condition editors
as one temporary `contenteditable` marker node over the exact safe bounds. The
controller binds the input kind and card to the open context menu. Missing
markers submit only the characterized default (`10` or `P`); existing markers
open without a command. Damage accepts bounded integers, while condition text
...
The damage and condition controls now open one controller-bound temporary React
...
`H`/`D`/`L`/Space, `A`/`B`, `G`, and non-Alt `P` key/code pairs to one typed
request carrying the selected stable card ID. The controller rechecks selection,
ready/live-player policy, and current recipient-safe membership, then delegates to the existing bounded
...
collapse a multi-card zone grant, and a normally known card has no private
grant to close. `Z` and Alt-`Z` resolve to explicit false/true public-reveal
targets, so duplicate values fail as no-ops. Forged, stale, unsupported,
...
The next five non-Alt moves send the card to its current board-side hand,
discard, lost zone, loose board, or prizes. They share one resolver with the
existing context-menu `to Board` path, retain stable source/work-area preconditions,
reject lower evolutions and same-zone requests, and dismiss selection only
after acceptance. Active/bench and stadium remain separate because they require
placement or singleton-replacement policy. Prize movement can reuse the generic
source resolver because the domain already treats prizes as concealed: the move
...
owner. The connected Solo header tab retains its separate exact v1 confirmation
and reuses that teardown, while the selected Replay tab cannot invoke it. Live
and replay Options restore Export battle log and Full screen; Clear
...
and replay mode. The live Solo Options menu wires its source-shaped `Enter replay
mode` control and hidden `.json` picker to the raw-byte replay validator and
atomic coordinator installation. Native selection, malformed-file recovery,
...
query change. Physical tab freezing/BFCache and other browsers remain release
evidence.
