...
- Extend the implemented room/session/seat capability lifecycle and version
  ...
  Perspective replay export and byte-safe, atomically installed replay-file
  import are implemented. The live Solo Options menu wires the source-shaped
  `Enter replay mode` control and hidden `.json` picker; canonical server-held
  continuation remains later work. Deck navigation is live behind the same
...
changing the effective board. The import boundary owns bounded raw bytes before
asynchronous work, rejects malformed UTF-8, validates the exact file-v1
envelope/integrity/privacy/semantics, and installs playback atomically only if
its initiating live identity is still current. The live Solo Options menu now
wires that transaction through the source-shaped `Enter replay mode` control and
...
opponent-invitation copy. A dedicated same-origin built Worker/Chromium journey
now exercises that complete file/clipboard path with command-line-only test
activation, verifies exact checkpoint preservation and source/target isolation,
and confirms completed-save revocation and browser bearer non-persistence.
Production key/quota-capacity provisioning, route activation, and managed
recovery/abuse/browser gates remain closed. The
