 ...
 *
 * `legacy-old-card-types.json` is regenerated by
 * `scripts/generate-legacy-old-card-types.mjs`, which replaces v1's
 * `find-old-type-database_updater.py` and reads the same upstream corpus
 * (https://github.com/PokemonTCG/pokemon-tcg-data). Its `--check` mode
 * reports any committed threshold a newer corpus would contradict, which is
 * the part that must never change: it would reclassify cards in saved decks.
 */
