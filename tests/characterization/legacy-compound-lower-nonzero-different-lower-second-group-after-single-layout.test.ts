...
import preDivergenceOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-group-single-v1.json';
import predecessorOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-group-after-single-v1.json';
...
import oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-second-group-after-single-v1.json';
...
describe('legacy lower nonzero-group repeated different-lower rotation after a lower single', () => {
  ...
  it('inherits every pre-state and complete trace prefix exactly from checkpoint 25', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
  ...
  it('proves checkpoint-26, internal q1/q3, and checkpoint-18 q2 geometry collisions, then closes lifecycle and exclusions', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
      ...
      expect(successorReference).toEqual({
        dependencyPath:
          'tests/legacy-fixtures/renderer/compound-lower-nonzero-same-lower-second-group-after-single-v1.json',
      ...
      if (definition.originalGroupTurns === 2) {
        ...
        for (const slot of slots) {
          ...
          expect(ownPost.slice(1)).toEqual(
            translatePhaseX(referencePre, delta).slice(1)
          );
        }
      } else {
    ...
    expect(included).toContain('same other lower initiator');
    expect(included).toContain('preserving all per-card BREAK flags');
    expect(included).toContain('-0.015625 px and +0.015625 px');
    expect(excluded).toContain('prior single-card lower');
