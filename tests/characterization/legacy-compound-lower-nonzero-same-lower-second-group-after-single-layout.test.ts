...
import predecessorOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-same-lower-group-after-single-v1.json';
import oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-same-lower-second-group-after-single-v1.json';
...
describe('legacy lower nonzero-group repeated same-lower rotation after a lower single', () => {
  ...
  it('inherits every pre-state and complete trace prefix exactly from checkpoint 24', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
  ...
  it('proves internal q1/q3 and checkpoint-18 q2 geometry collisions, then closes lifecycle and exclusions', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
      const roleTitle =
