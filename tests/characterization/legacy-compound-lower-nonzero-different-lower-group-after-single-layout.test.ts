...
import oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-group-after-single-v1.json';
import sameLowerReferenceOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-same-lower-group-after-single-v1.json';
...
const topReferenceTraces = topReferenceOracle.expected
  ...
  readonly string[]
>;
const sameLowerReferenceEvidence = sameLowerReferenceOracle.expected
  .phaseEvidenceByScenarioAndSlot as unknown as Record<
  `${SameLowerReferenceScenario}:${Slot}`,
  readonly [PhaseEvidenceTuple, PhaseEvidenceTuple]
>;
const sameLowerReferenceTurns = sameLowerReferenceOracle.expected
  .quarterTurnsByScenario as unknown as Record<
  SameLowerReferenceScenario,
  readonly [Record<Role, number>, Record<Role, number>]
>;
const sameLowerReferenceFlags = sameLowerReferenceOracle.expected
  .breakFlagsByScenario as unknown as Record<
  SameLowerReferenceScenario,
  readonly [Record<Role, boolean>, Record<Role, boolean>]
>;
const sameLowerReferenceTraces = sameLowerReferenceOracle.expected
  .operationTraceByScenario as unknown as Record<
  SameLowerReferenceScenario,
  readonly string[]
>;
...
const sameLowerReference = (
  ...
  const title = definition.priorLowerRole === 'middle' ? 'Middle' : 'Base';
  const scenario =
    `${definition.composition}${title}GroupAfter${title}SingleAtGroupQ${definition.originalGroupTurns}` as SameLowerReferenceScenario;
  return {
    scenario,
    evidence: sameLowerReferenceEvidence,
    turns: sameLowerReferenceTurns[scenario],
    flags: sameLowerReferenceFlags[scenario],
    trace: sameLowerReferenceTraces[scenario],
  };
};
...
describe('legacy lower nonzero-group different-lower rotation after a lower single', () => {
  it('closes direct source provenance and all four recursive semantic dependencies', () => {
  ...
  it('pins the exact unique 48-case, 12-scenario, two-composition matrix and metadata', () => {
    ...
    for (const actual of [
      ...
      Object.keys(oracle.expected.sameLowerInitiatedReferenceByScenario),
      Object.keys(turns),
      Object.keys(flags),
      Object.keys(traces),
    ...
    for (const scenario of scenarios) {
      ...
      expect(metadata.measuredRole).not.toBe(metadata.priorLowerRole);
      expect(metadata.measuredIndex).not.toBe(metadata.priorLowerIndex);
      expect(metadata.measuredDomOrdinal).not.toBe(
        metadata.priorLowerDomOrdinal
      );
    }
  });
  ...
  it('pins independent whole-group turns, unchanged flags, margins, geometry, traces, and all ten probes', () => {
    for (const scenario of scenarios) {
      ...
      expect(flags[scenario][1][definition.measuredRole]).toBe(false);
  ...
  it('proves checkpoint-22 and -24 successor equality with both signed margin differentials, then closes lifecycle and exclusions', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
      const top = topReference(definition);
      const same = sameLowerReference(definition);
      const recordedTop = oracle.expected.topInitiatedReferenceByScenario[
        scenario
      ] as {
      ...
      const recordedSame = oracle.expected
        .sameLowerInitiatedReferenceByScenario[scenario] as {
        readonly dependencyPath: string;
        readonly scenario: string;
        readonly activeFrameLocalXDelta: number;
        readonly benchFrameLocalXDelta: number;
      ...
      const topBenchDelta =
        definition.composition === 'ordinary'
          ...
          : definition.originalGroupTurns === 2
            ? -0.015625
            : 0.015625;
      const sameBenchDelta = definition.originalGroupTurns === 2 ? 0 : 0.015625;
      expect(recordedTop).toEqual({
        dependencyPath:
          'tests/legacy-fixtures/renderer/compound-lower-nonzero-group-rotation-after-single-v1.json',
        scenario: top.scenario,
        activeFrameLocalXDelta: 0,
        benchFrameLocalXDelta: topBenchDelta,
      ...
      expect(recordedSame).toEqual({
        dependencyPath:
          'tests/legacy-fixtures/renderer/compound-lower-nonzero-same-lower-group-after-single-v1.json',
        scenario: same.scenario,
        activeFrameLocalXDelta: 0,
        benchFrameLocalXDelta: sameBenchDelta,
        measuredRole: definition.measuredRole,
        measuredIndex: definition.measuredIndex,
        referenceMeasuredRole: definition.priorLowerRole,
        referenceMeasuredIndex: definition.priorLowerIndex,
      });
      expect(turns[scenario][1]).toEqual(top.turns[1]);
      expect(turns[scenario][1]).toEqual(same.turns[1]);
      expect(flags[scenario][1]).toEqual(top.flags[1]);
      expect(flags[scenario][1]).toEqual(same.flags[1]);
      expect(traces[scenario]).not.toEqual(top.trace);
      expect(traces[scenario]).not.toEqual(same.trace);
      expect(transitions[scenario]).not.toBe(top.trace.at(-1));
      expect(transitions[scenario]).not.toBe(same.trace.at(-1));
      for (const slot of slots) {
        const ownPost = phaseEvidence[`${scenario}:${slot}`][1];
        const topPost = top.evidence[`${top.scenario}:${slot}`]?.[1];
        const samePost = same.evidence[`${same.scenario}:${slot}`]?.[1];
        const topDelta = slot === 'active' ? 0 : topBenchDelta;
        const sameDelta = slot === 'active' ? 0 : sameBenchDelta;
        expect(ownPost.slice(1)).toEqual(
          translatePhaseX(
            required(topPost, `${scenario}.${slot}.top-reference`),
            topDelta
          ).slice(1)
        );
        expect(ownPost.slice(1)).toEqual(
          translatePhaseX(
            required(samePost, `${scenario}.${slot}.same-reference`),
            sameDelta
          ).slice(1)
        );
        if (slot === 'bench') {
          expect(margins[`${scenario}:${slot}`][1]).toEqual(
            definition.originalGroupTurns === 2 ? ['3%', '2%'] : ['1%', '0%']
          );
        }
      }
    ...
    expect(included).toContain('other lower card');
    expect(included).toContain('preserving all per-card BREAK flags');
    expect(included).toContain('signed frame-local bench translations');
    expect(excluded).toContain('same lower card');
