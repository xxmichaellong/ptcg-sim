...
import differentLowerSecondOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-second-group-after-single-v1.json';
import priorLowerOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-top-then-prior-lower-group-after-single-v1.json';
import oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-top-then-other-lower-group-after-single-v1.json';
...
describe('legacy lower nonzero-group top-then-other-lower rotation after a lower single', () => {
  ...
  it('pins the exact unique 48-case, 12-scenario, two-composition matrix and metadata', () => {
    ...
    for (const scenario of scenarios) {
      ...
      expect(scenarioMetadata[scenario]).toEqual({
        ...
        measuredRole: measuredRole(scenarioDefinitions[scenario]),
        measuredIndex: measuredIndex(scenarioDefinitions[scenario]),
        measuredDomOrdinal: measuredDomOrdinal(scenarioDefinitions[scenario]),
        measuredGroupRotationOrdinal: 2,
      });
  ...
  it('pins independent whole-group turns, unchanged flags, margins, geometry, traces, and all ten probes', () => {
    for (const scenario of scenarios) {
      ...
      expect(flags[scenario][1][measuredRole(definition)]).toBe(false);
  ...
  it('proves checkpoint-27 exact posts, checkpoint-29 bounded comparisons, and internal q1/q3 collisions, then closes lifecycle and exclusions', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
    ...
    expect(included).toContain('other lower sibling');
    expect(included).toContain('preserving all per-card BREAK flags');
    expect(included).toContain('checkpoint 27');
    expect(excluded).toContain('top or prior divergent lower');
