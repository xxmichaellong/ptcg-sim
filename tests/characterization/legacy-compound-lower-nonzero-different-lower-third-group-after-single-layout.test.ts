...
import checkpoint25Oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-group-after-single-v1.json';
import predecessorOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-second-group-after-single-v1.json';
import checkpoint32Oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-same-lower-third-group-after-single-v1.json';
import oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-third-group-after-single-v1.json';
...
describe('legacy lower nonzero-group repeated different-lower rotation after a lower single', () => {
  ...
  it('inherits every pre-state and complete trace prefix exactly from checkpoint 27', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
  ...
  it('proves checkpoint-25 and checkpoint-32 comparisons plus internal q1/q3 collisions, then closes lifecycle and exclusions', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
      ...
      for (const slot of slots) {
        const delta = slot === 'bench' ? checkpoint32BenchDelta : 0;
        expect(phaseEvidence[`${scenario}:${slot}`][1].slice(1)).toEqual(
          translatePhaseX(
