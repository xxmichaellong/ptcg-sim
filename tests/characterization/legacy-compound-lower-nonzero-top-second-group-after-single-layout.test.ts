...
import otherLowerSecondOracle from '../legacy-fixtures/renderer/compound-lower-nonzero-different-lower-second-group-after-single-v1.json';
import oracle from '../legacy-fixtures/renderer/compound-lower-nonzero-top-second-group-after-single-v1.json';
...
describe('legacy lower nonzero-group repeated top rotation after a lower single', () => {
  ...
  it('proves checkpoint-27, internal q1/q3, and checkpoint-18 q2 geometry collisions, then closes lifecycle and exclusions', () => {
    for (const scenario of scenarios) {
      const definition = scenarioDefinitions[scenario];
      ...
      if (definition.originalGroupTurns === 2) {
        ...
        for (const slot of slots) {
          ...
          expect(ownPost.slice(1)).toEqual(referencePre.slice(1));
        }
      } else {
    ...
    expect(included).toContain('same top initiator');
    expect(included).toContain('preserving all per-card BREAK flags');
    expect(included).toContain('signed 0.015625 px');
    expect(excluded).toContain('lower card or different initiator');
    expect(excluded).toContain('third or later top group rotation');
