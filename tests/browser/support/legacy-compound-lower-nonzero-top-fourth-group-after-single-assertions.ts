...
import checkpoint18Oracle from '../../legacy-fixtures/renderer/compound-lower-nonzero-group-single-v1.json' with { type: 'json' };
import checkpoint28Oracle from '../../legacy-fixtures/renderer/compound-lower-nonzero-top-second-group-after-single-v1.json' with { type: 'json' };
import predecessorOracle from '../../legacy-fixtures/renderer/compound-lower-nonzero-top-third-group-after-single-v1.json' with { type: 'json' };
import oracle from '../../legacy-fixtures/renderer/compound-lower-nonzero-top-fourth-group-after-single-v1.json' with { type: 'json' };
...
export type LowerNonzeroTopFourthGroupAfterSingleComposition =
  'ordinary' | 'break';
...
interface ScenarioDefinition {
  readonly composition: LowerNonzeroTopFourthGroupAfterSingleComposition;
...
const scenarioOrderByComposition = {
  ...
  LowerNonzeroTopFourthGroupAfterSingleComposition,
  readonly Scenario[]
>;
...
const expectedCaseIds = (
  composition: LowerNonzeroTopFourthGroupAfterSingleComposition
): readonly string[] => {
  const ids: string[] = [];
...
export const assertLowerNonzeroTopFourthGroupAfterSingleOracleIntegrity =
  async (
    composition: LowerNonzeroTopFourthGroupAfterSingleComposition
  ): Promise<void> => {
    ...
    for (const scenario of allScenarios) {
      ...
      for (const slot of slots) {
        expect(
          ...
          `${scenario}:${slot}.checkpoint-28-post-collision`
        ).toEqual(
      ...
      for (const slot of slots) {
        ...
        expectCollisionPhase(
          ...
          xDelta,
          `${scenario}:${slot}.checkpoint-18-post-comparison`
        );
        if (xDelta === 0) {
          expect(margins[`${scenario}:${slot}`][1]).toEqual(
    ...
    expect(oracle.scope.included.join(' ')).toContain(
      'immediate measured fourth whole-group action'
    );
...
export const assertLowerNonzeroTopFourthGroupAfterSingleLiveCapture = async (
  page: Page,
  testInfo: TestInfo,
  composition: LowerNonzeroTopFourthGroupAfterSingleComposition,
  ...
  await testInfo.attach(
    `legacy-source-compound-lower-nonzero-top-fourth-group-after-single-${composition}.json`,
    {
  ...
  expect(
    capture.lowerNonzeroTopFourthGroupAfterSingleCases.map((entry) => entry.id)
  ...
  for (const entry of capture.lowerNonzeroTopFourthGroupAfterSingleCases) {
    ...
    const prePhase = required(
      entry.phases[0],
      `${entry.id}.pre-fourth-group-rotation`
    );
    const postPhase = required(
      entry.phases[1],
      `${entry.id}.post-fourth-group-rotation`
    );
    ...
    expect(divergentPost.localRotationDegrees).toBe(0);
