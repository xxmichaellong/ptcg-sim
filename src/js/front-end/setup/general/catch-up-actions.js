...
export const catchUpActions = (actionData) => {
    ...
    if (mostRecentDeckDataEntry){
        acceptAction('opp', mostRecentDeckDataEntry.action, mostRecentDeckDataEntry.parameters);
    };
    if (mostRecentResetAndAfterEntries && mostRecentResetEntryIndex < mostRecentDeckDataIndex){
        mostRecentResetAndAfterEntries.forEach(data => acceptAction('opp', data.action, data.parameters));
    } else if (mostRecentDeckDataEntry){
        entriesAfterMostRecentDeckData.forEach(data => acceptAction('opp', data.action, data.parameters));
    } else {
        missingData.forEach(data => acceptAction('opp', data.action, data.parameters));
    };
}