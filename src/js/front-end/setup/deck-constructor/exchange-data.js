...
export const exchangeData = (user, username, deckData, cardBack, coachingMode, callback = true, emit = true) => {
    ...
    if (user === 'self'){
        ...
        if (callback){
            exchangeData('self', systemState.p2SelfUsername, systemState.selfDeckData, systemState.cardBackSrc, coachingModeCheckbox.checked, false);
        };
    };

    processAction(user, emit, 'exchangeData', [username, deckData, cardBack, coachingMode, callback]);
}