...
export const dragOver = (event) => {
    ...
    const targetParentIsActiveOrBench = ['active', 'bench'].includes(event.target.parentElement.parentElement.id);
    const targetNotItself = event.target !== draggedImage;
    const targetIsAttached = event.target.attached;
    const cardIsFromActiveOrBench = ['active', 'bench'].includes(draggedImage.parentElement.parentElement.id);
    ...
    if (targetIsNotPlayContainer && (movingValidCardToContainer || attachingValidCard)){
...
export const dragEnd = (event) => {
    ...
    if (mouseClick.playContainer){
        ...
        mouseClick.playContainerParent.style.zIndex = mouseClick.playContainer.parentElement ? '2' : '0';
        document.getElementById('stadium').style.zIndex = mouseClick.playContainer.parentElement ? '-1' : '0'; //crucial line to bring stadium z-index back to 0 if the pokemon is completely put out of play
...
export const drop = (event) => {
    ...
    if (targetIsNotPlayContainer && draggedImage.layer !== undefined && (!event.target.attached || event.target.tagName === 'DIV')){
        ...
        if (event.target.tagName === 'IMG' && event.target !== draggedImage[0] && ['active', 'bench'].includes(event.target.parentElement.parentElement.id)){
            dZoneId = event.target.parentElement.parentElement.id;
            targetIndex = getZone(event.target.user, dZoneId).array.findIndex(card => card.image === event.target);
