...
export const moveCard = (user, initiator, oZoneId, dZoneId, index, targetIndex) => {
    ...
    const isTargetCardValid = targetCard && activeOrBenchZone.includes(dZoneId) && !targetCard.image.attached;
    const isAttachAllowed = !activeOrBenchZone.includes(oZoneId) || movingCard.image.attached;
    ...
    if (!['active', 'board', 'bench', 'attachedCards'].includes(dZoneId) && movingCard.type2){
