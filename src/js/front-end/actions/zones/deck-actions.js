...
export const shuffleIntoDeck = (user, initiator, zoneId, index, indices, emit = true) => {
    const oInitiator = initiator === 'self' ? 'opp' : 'self';
    if (user === 'opp' && emit && systemState.isTwoPlayer){
        processAction(user, emit, 'shuffleIntoDeck', [oInitiator, zoneId, index, indices]);
        return;
    };

    moveCardBundle(user, initiator, zoneId, 'deck', index, false, 'shuffle', false);
...
export const viewDeck = (user, initiator, viewAmount, top, selectedDeckCount, targetIsOpp, emit = true) => {
    const oInitiator = initiator === 'self' ? 'opp' : 'self';
    if (user === 'opp' && emit && systemState.isTwoPlayer){
        processAction(user, emit, 'viewDeck', [oInitiator, viewAmount, top, selectedDeckCount, targetIsOpp]);
        return;
    };
...
export const switchWithDeckTop = (user, initiator, oZoneId, index, emit = true) => {
    const oInitiator = initiator === 'self' ? 'opp' : 'self';
    if (user === 'opp' && emit && systemState.isTwoPlayer){
        processAction(user, emit, 'switchWithDeckTop', [oInitiator, oZoneId, index]);
        return;
    };

    if (oZoneId !== 'deck' && oZoneId !== 'deckCover'){
        moveCardBundle(user, initiator, oZoneId, 'deck', index, false, 'switch', false);  //first part is moving a card to the top of the deck
